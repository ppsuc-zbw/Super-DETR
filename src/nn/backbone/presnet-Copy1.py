...
####block是一个传入的参数，该部分是网络的基本结构，count为网络的block个数
...
class PResNet(nn.Module):
    def __init__(
        ...
        # print(block.expansion,'expansion')###expansion 是直接写死在block中的扩张倍数
    ...
    def forward(self, x):
        ...
        #首先经过了三层卷积，卷积前后图像尺寸不发生变化
        conv1 = self.conv1(x)
        # x = nn.Conv2d(conv1, kernel_size=3, stride=2, padding=1)
        x = F.max_pool2d(conv1, kernel_size=3, stride=2, padding=1) #下采样后图片
        x_stage=x ###用于后续循环
        outs = []
